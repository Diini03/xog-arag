...
export default function Register() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    toast.success("Account created! Welcome to XogArag.");
    nav("/");
  };
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="shadow-elevated border-2">
          <CardContent className="p-8">
            ...
            <p className="text-sm text-muted-foreground mb-6">Start turning data into insights in minutes.</p>
            <form onSubmit={onSubmit} className="space-y-4">
              <div>
                <Label htmlFor="name">Full name</Label>
                <Input id="name" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Ada Lovelace" className="mt-1.5" />
              </div>
              <div>
